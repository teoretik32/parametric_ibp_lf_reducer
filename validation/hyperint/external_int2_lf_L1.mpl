# External Int2 Method.12A -- exact HyperInt input for LF master L1
# label [-1, 0, 0, -1, -1, 0, 0]; certified basis unchanged; audit artifact only.
# REVIEW BEFORE RUNNING: prepared input; do NOT start a long integration
# unattended.  Uncomment the hyperInt call after review.
read "HyperInt.mpl":
_hyper_verbosity := 0:
epsOrder := 2:  # expand through ep^2
f := x2^(ep) * (x2 + 1)^(ep - 1) * (x5 + 1)^(ep - 1) * (x7 + 1)^(-ep - 1) * (r*x2*x5 + x2*x7 + x7 + 1)^(ep - 1):

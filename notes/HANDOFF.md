...
- Stage-3 (30 точек = Stage-2 + 9 генерических,
  `validation/method11b_samples_stage3.json`; cache-warm 63/90 hits, досчитано
  27 записей, итог 8259.5 s): вердикт снова `Failure(InterpolationFailed)`,
  n_terms=0 — ёмкостный отказ. Гейт = PROCEED
  (`validation/method11b_stage3_gate.json`): 90/90 formal_success, единый ранг
  26984 (0 провалов), 29 stable + 1 special-zero (тот же `ep=6,r=57/11`, лейбл
  `(0,0,1,-1,0,0,-1)`), покрытие 30/30, v2-заголовок ок. Вывод: степени
  коэффициентов выше (3,3) — 29 генерических точек недостаточно.
- Stage-4 запущен в фоне: `validation/method11b_samples_stage4.json` — 38 точек
  (суперсет Stage-3 + 8 генерических `ep=114/7..138/7` шаг 3/7, целая
  `126/7=18` пропущена; r из свободных числителей `33..60/11`). 37 генерических
  точек покрывают ёмкостные минимумы (4,4)=31 и (5,4)=36. Кэш-тёплый старт
  90/114, досчитать 24 записи. Явный `--samples-file` обходит ограничение
  `--n-samples <= 37` (оно только про генератор). Гейт после завершения:
  `--expected-points 38`.
## External Int2 Method.11b Stage-4: STOP + корневой диагноз — ёмкость модуля (2026-07-25)

- Stage-4 завершён (38 точек × 3 prime, cache-warm 90/114, досчитано 24 записи):
  вердикт раннера `Failure(InterpolationFailed)`, n_terms=0. Гейт = STOP
  (`validation/method11b_stage4_gate.json`): 114/114 formal_success, единый ранг
  26984 (0 провалов), support 4, 37 stable + 1 special-zero (тот же
  `ep=6,r=57/11`) — «non-capacity failure»: сэмплинг-ёмкость исчерпана, отказ
  остался.
- Офлайн-диагноз из кэша (без нового сэмплинга,
  `validation/method11b_reconstruction_diag.json`): падение НЕ в денс-анзаце, а
  в подъёме значений из остатков: `collect_value_table` →
  `reconstruct_rational()` (reconstruction.py:67) требует стабилизации —
  кандидат по k простым должен совпасть с кандидатом по k−1. Для лейбла
  `(0,0,0,-1,0,0,-1)` в точке `ep=120/7, r=52/11` истинное значение
  `346424213/1648857600`; знаменатель 1 648 857 600 (`2^9·3^3·5^2·13·367`)
  превышает 2-prime Wang-предел 1 518 500 221 → 2-prime кандидат None, 3-prime
  кандидат верен, но сравнивать не с чем → «never stabilized → need more
  primes».
- Вывод: корневая причина STOP — ёмкость модуля (число простых), а не
  сэмплинг/ранги/суппорт. Гейтовая метка «non-capacity» покрывает только оси
  rank/support; prime-count как ось ёмкости гейт не измеряет (кандидат на
  доработку гейта).
- Следующий шаг (тяжёлый прогон — только после одобрения): Stage-4 с 4-м
  простым: `--primes 2147483647,2147483629,2147483587,2147483579` + тот же
  `--samples-file validation/method11b_samples_stage4.json`; кэш переиспользует
  все 114 записей (ключ = точка×prime), досчитать нужно только 38 редукций
  нового prime (~1/3 ступени). После certified Success — обязательный Phase D:
  экспорт reduction text + сверка с Laurent-оракулом.

## External Int2 Method.11b Stage-4 + 4-й prime: гипотеза «ёмкость модуля» опровергнута — ёмкость по точкам (2026-07-26)

- Stage-4 повторён с `--primes 2147483647,2147483629,2147483587,2147483579`
  (кэш-тёплый: 114 записей переиспользованы, досчитано 38 редукций нового
  prime; 11071.5 s): вердикт раннера снова `Failure(InterpolationFailed)`,
  n_terms=0 (`validation/external_int2_method11b_stage4p4.json`).
- Solve-фаза чистая: 152/152 formal_success, единый ранг 26984 (0 rank drops),
  покрытие 38/38 × 4 prime, `points_incomplete_primes=[]`; support 4,
  37 stable + 1 special-zero (тот же `ep=6,r=57/11`, retained/zero-filled).
  Гейт (`validation/method11b_stage4p4_gate.json`): **PROCEED**, reasons:
  «capacity-bound failure» + special-zero.
- Гипотеза Stage-4 «корень — ёмкость модуля (CRT/Wang, нужен 4-й prime)»
  ОПРОВЕРГНУТА: с 4-м prime подъём значений стабилизировался (лестница дошла
  до денс-анзацев), но интерполяция лейбла `(-1,0,0,-1,-1,0,0)` (38 точек =
  36 fit + 2 holdout) не нашла решения: 49 ячеек лестницы степеней до (6,6) —
  33× «no unique nullspace solution (dim 0)» (несовместно ⇒ истинная степень
  выше ячейки), 8× dim 2/5 (неоднозначно), остальные underdetermined
  (нужно 37..55 fit-точек, есть 36).
- Новый корневой диагноз: ёмкость сэмплинга по точкам — истинные степени
  коэффициента выше всех определённых 36 точками ячеек; для ячейки (6,6)
  нужно ≥55 fit-точек (≥57 сэмплов при holdout 2). Это согласуется с
  эскалацией Stage-3 (степени выше (3,3)) → Stage-4 (выше покрытых ячеек).
- Phase D (сверка с Laurent-оракулом) не выполнялся — нет certified Success.
- Следующий шаг (тяжёлый прогон — ТОЛЬКО после одобрения): Stage-5 с ≥57
  точками (суперсет Stage-4; кэш переиспользует все 152 записи), либо решение
  о смене анзаца/базиса. Без одобрения ничего не запускать.

### External Int2 Method.11c: офлайн-аудит реконструкции (без RREF, из кэша)

- `scripts/audit_external_int2_reconstruction.py` (+13 тестов
  `tests/test_reconstruction_audit.py`): фазы A–D поверх 152 кэш-записей;
  никаких новых модульных редукций, `sparse_rref`/reducer не импортируются.
- Phase A: `validation/external_int2_method11_value_table.json` — точная
  таблица 38×4 (CRT-значения + остатки на каждом prime + special-zero
  zero-fill + ключи кэша); верификация OK (значение ≡ остатку mod все 4 prime,
  единый ранг 26984, support согласован по primes).
- Phase B: офлайн повторяет лестницу Stage-4 в точности
  (`artifact_crosscheck.matches=true`); dim-0 mod p ⇒ dim-0 над QQ
  (доказательные несовместности). Первая неопровергнутая dense-ячейка у всех
  4 лейблов — (3,6) (38 неизвестных, нужно 37 fit, есть 36).
- Phase C (все отрицательные, честно записаны): унивариатные фибры
  невозможны (38 различных ep и 38 различных r); 610 tensor-ячеек/лейбл —
  все dim-1 кандидаты отвергнуты точной QQ-проверкой; factor-aware — формы
  `(3·(7ep)+7)`, `(11r)`, … покрывают знаменатели, но value·D не полином
  deg ≤ 7; shared-denominator — 50 определённых ячеек dim 0 (фронтир a≤7,
  b≤12); кусочная/chamber гипотеза (G) — 32 границы, ни одного двустороннего
  точного фита. Ни один из 4 коэффициентов не реконструирован из 38 точек.
- Phase D: минимумы — dense (3,6) +1 точка; shared-frontier (7,1) +1;
  factor-aware deg-8 +8. Рекомендованный детерминированный пакет: 8 точек
  (фибры ep=15/7: r=61..65/11; ep=18/7: r=66..68/11), seed 20260726,
  исключены ep∈{3,6}, r=57/11, полюсы форм. Артефакт:
  `validation/external_int2_method11_reconstruction_audit.json`; заметка:
  `notes/EXTERNAL_INT2_RECONSTRUCTION_AUDIT.md`.
- Вывод для Stage-5: 38 точек недостаточно ни для одного структурного анзаца;

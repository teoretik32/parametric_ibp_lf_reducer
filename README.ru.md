...
- **Adaptive search — ограниченное расписание, а не прувер** — opt-in `--adaptive`
  запускает детерминированную эскалацию фиксированных проходов
  ([docs/ADAPTIVE_SEARCH.ru.md](docs/ADAPTIVE_SEARCH.ru.md)); его исчерпание не
  доказывает отсутствие редукции. Без флага — прежний один фиксированный проход.

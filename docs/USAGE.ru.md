...
`examples/example4_star_corrected_input.wl.txt` исправляет подынтегральное
выражение: множитель `15*ep + 24*ep*x7` обрабатывается по линейности как два
прогона семейства, `15*ep*J[{0,0,0,0,0,0,0}] + 24*ep*J[{0,1,0,0,0,0,0}]`,
с объединением результатов в `scripts/run_example4_star_corrected.py`.
Исправленный прогон вернул сертифицированный `Success` (certificate `Passed`,
`selected_rank` 9924) — артефакты:
`validation/example4_star_corrected_result.m`,
`validation/example4_star_corrected_diagnostics.json`; тесты:
`tests/test_example4_star_corrected.py`. По-прежнему exploratory
(known-value-only, не часть certified baseline).

## Выбор RREF-бэкенда (`--rref-backend`, v0.1.4)

Доминирующая стоимость тяжёлых прогонов — модулярное RREF-ядро. v0.1.4
добавляет опциональный Numba-бэкенд:

```bash
pip install -e .             # базовая установка — только dict-бэкенд
pip install -e ".[speed]"    # добавляет numba для ускоренного бэкенда
```
...
- **Дефолт по-прежнему `dict`** (чистый Python, без доп. зависимостей);
  без `--rref-backend` поведение не отличается от прошлых релизов.
- **`auto` — рекомендуемый opt-in для больших систем** при установленном
  extra `[speed]`. Выбор происходит по-матрично: Numba берётся, только если
  она доступна, `prime < 2^31` (гарантия int64) и матрица проходит все
  пороги (сейчас `min_rows=500`, `min_cols=400`, `min_nnz=3000`).
  Малые системы обычно остаются на `dict`.
- `auto` **тихо откатывается на `dict`**, если Numba недоступна.
- **Явный** запрос `numba_int_array_experimental` никогда не подменяется:
  при отсутствии Numba или неподходящем prime он падает с понятной ошибкой.
- Выбор бэкенда влияет только на скорость: результаты идентичны между
  бэкендами; решение фиксируется в диагностике
  (`requested_rref_backend`, `selected_rref_backend`,
  `backend_selection_reason`, `numba_available`, `auto_thresholds_used`).
- Сертифицированный бенчмарк (Perf.13, corrected Example 4\*, полный бокс —
  972 лейбла, 12360 строк, ранг 9924, 36/36 записей): wall 3963.4с (`dict`)
  → 803.8с (явная Numba) → 766.5с (`auto`), `rref_mod_p` 3124.1с → 656.1с;
  выходы идентичны, `Success`, `AllLocallyFinite=True`, сертификат `Passed`,
